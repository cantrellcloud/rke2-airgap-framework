...
curl -fsSLo "$RKE2_LIST" "https://github.com/rancher/rke2/releases/download/${RKE2_SAFE}/rke2-images.${ARCH}.txt"
...
while read -r IMG; do
  ...
  if [ -f "$OUT" ]; then echo "[SKIP] $IMG"; continue; fi
  echo "[PULL] $IMG"
  skopeo copy --all --retry-times 3 "docker://${IMG}" "oci-archive:${OUT}:${IMG##*:}"
done < "$ALL_LIST"
