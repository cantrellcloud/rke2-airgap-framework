...
CA_SRC=""; ROLE="auto"; NO_RESTART="false"
while (( "$#" )); do
  case "$1" in
...
mirrors:
  "docker.io":        { endpoints: ["https://${REGISTRY_ROOT}/docker.io"] }
  "registry.k8s.io":  { endpoints: ["https://${REGISTRY_ROOT}/registry.k8s.io"] }
  "quay.io":          { endpoints: ["https://${REGISTRY_ROOT}/quay.io"] }
  "ghcr.io":          { endpoints: ["https://${REGISTRY_ROOT}/ghcr.io"] }
  "rancher":          { endpoints: ["https://${REGISTRY_ROOT}/rancher"] }
  "rancher.io":       { endpoints: ["https://${REGISTRY_ROOT}/rancher.io"] }
configs:
  "${REG_HOST}":
    auth:
...
ROLE_RESOLVED="${ROLE}"
if [[ "$ROLE" == "auto" ]]; then
  if systemctl list-unit-files | grep -q '^rke2-server\.service'; then ROLE_RESOLVED="server"; else ROLE_RESOLVED="agent"; fi
fi
